...
from Evaluation.labelbasedclassification import accuracyMacro, accuracyMicro, precisionMacro, precisionMicro, \
    recallMacro, recallMicro, fbetaMacro, fbetaMicro
import numpy as np
import pandas as pd
...
def get_classification_metrics(result):
    ...
    macro = sum(roc_auc.values()) / (trueLabels.shape[1])
    fpr = dict()
    tpr = dict()
    roc_auc = dict()
    for i in range(trueLabels.shape[1]):
