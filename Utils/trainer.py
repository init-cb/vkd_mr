    ...
    for epoch in range(n_epochs):
        with tqdm(total=batch_size * len(train_loader)) as epoch_pbar:
            ...
            for i, (x1_batch, x2_batch, y_batch) in enumerate(train_loader):
                # 首先将数据移至 GPU 上，并使用模型进行预测。然后计算损失并进行反向传播。
                ...
                loss_dict = model.loss_function(y_pred, y_batch, mu, logvar, y_predr, mur, logvarr, kl_ann_factor[i])
                loss = loss_dict['loss']
                optimizer.zero_grad()
                loss.backward()
        ...
        with tqdm(total=batch_size * len(valid_loader)) as epoch_pbar:
            ...
            for i, (x1_batch, x2_batch, y_batch) in enumerate(valid_loader):
                with torch.no_grad():
                    ...
                    loss_dict = model.loss_function(y_pred, y_batch, mu, logvar, y_predr, mur, logvarr,
                                                    kl_ann_factor[2])
        ...
        print('VAL epoch {}/{} \t loss={:.4f} \t val_loss={:.4f} \t time={:.2f}s'.format(epoch + 1, n_epochs, avg_loss,
    ...
    with torch.no_grad():
        for i, (x1_batch, x2_batch, label) in tqdm(enumerate(test_loader)):
            if counter < test_len // batch_size:
                ...
                classes_ehr[counter * batch_size:(counter + 1) * batch_size] = F.sigmoid(
                    y_pred_ehr).detach().cpu().numpy()
