...
from transformers import BertTokenizer, BertModel, BertForMaskedLM, LEDTokenizer
import numpy as np
...
with open('config.json', 'r', encoding='utf-8') as json_f:
    ...
    JSON_FILE_PATH = config['pretrain_BERT_Path_JSON_FILE']
    STATE_DICT_PATH = config['pretrain_BERT_Path_STATE_DICT']
    TOKENIZER_PATH = config['pretrain_BERT_Path_TOKENIZER']
...
#     def __init__(
#             self,
#             model_path="...clinicalBERT/pretrained_bert_tf/biobert_pretrain_output_all_notes_150000",
#             padding=256,
#     ):
...
#                 "[PAD]" for _ in range(self.padding - len(tokenized_text))
#             ]
...
#         with torch.no_grad():
#             encoded_layers, hidden1, hidden2 = self.model(
...
class McbertEmbedding(object):
    ...
    def __init__(
        ...
        self.tokens = ""
        self.sentence_tokens = ""
        self.config = BertConfig.from_json_file(JSON_FILE_PATH)
    ...
    def eval_fwdprop_biobert(self, tokenized_text):
        ...
        # print(tokens_tensor)
        # print('segments_tensors:'+segments_tensors.tolist())

        # for i in self.model.children():
        #     print(i)

        with torch.no_grad():
            # encoded_layers, hidden1, hidden2 \
            outputs = self.model(
                tokens_tensor, segments_tensors
            ).to_tuple()
